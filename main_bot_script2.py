...
async def restart(ctx):
    ...
    try:
        ...
        # Replace 'python' with the appropriate command to start your bot. was replaced with $restart below
        # If you're using a virtual environment, include the activation command.
        subprocess.Popen(['$restart', 'main_bot_script.py'])  # i think its this file right
        await bot.close()  # hurls away current bot instance
    except Exception as e:
        await ctx.send(f"Error during restart: {e}")

# last one didn't work, all it did was just kick the bot offline. if this doesn't work then either somehow integrate javascript into this with the power of Bobby Stokes or... can it. 
...
def get_metar(icao, hoursback=0, format='json'):
    try:
        ...
        if 'rawText' in json_data[0]:
            raw_metar = json_data[0]['rawText']
        else:
            raise KeyError(f"Missing 'rawText' key in METAR data for {icao}")

        return raw_metar

    except requests.exceptions.RequestException as e:
        raise Exception(f"Error fetching METAR data for {icao}: {e}") from e  # Chain the original exception
    except (KeyError, ValueError) as e:
        raise Exception(f"Error parsing METAR data for {icao}: {e}") from e  # Chain the original exception
...
async def taf(ctx, airport_code: str):
    ...
    try:
        ...
        if 'rawText' in json_data[0] and 'issueTime' in json_data[0]:
            ...
            raise KeyError(f"Missing 'rawText' or 'issueTime' key in TAF data for {airport_code}")
...
async def skewt(ctx, station_code: str):
    ...
    try:
        ...
        today = datetime.date.today().strftime("%Y-%m-%d")  # Format changed to YYYY-MM-DD

        # Construct the URL using today's date and the new parameters
        ...
        sounding_pre_tag = soup.find("pre")

        if sounding_pre_tag:
            sounding_data = sounding_pre_tag.text.strip()
        else:
            raise ValueError(f"No sounding data found for station {station_code} on {today}. Check if the station code is valid or if data is available for today.")
...
# i think the problem with the SkewT section was i needed to import datetime and then write the script so it knows the date and time you want your sounding for the present, aka the last sounding to be released
...
# okay, sat command section works. just no fancy panty image. just shoots you a link for now. we will go from here.
...
async def lightning(ctx, icao: str, radius: int = 5):
    ...
    try:
        ...
        api_url = f'https://data.api.xweather.com/lightning/:auto?format=json&filter=cg&limit=10&lat={airport_coords[0]}&lon={airport_coords[1]}&radius={radius}&client_id={client_id}&client_secret={client_secret}'
